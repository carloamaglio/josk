...
/**
 * The <code>KeyboardLayout</code> class is a layout manager that
 ...
 * 				Carlo Amaglio - modified the original in order to use all the available pixels
 */
public class KeyboardLayout implements LayoutManager2 {
	int hgap = 0;								//horizontal gap
	int vgap = 0;								//vertical gap
	int borderSZ = 0;							// bordo SX/DX
	 ...
	 */
	public KeyboardLayout(Dimension gridSize) {
		this(gridSize, 0, 0);
	}

	public KeyboardLayout(int rows, int columns, int hgap, int vgap) {
		this(new Dimension(columns, rows), hgap, vgap);
	}

	public KeyboardLayout(int rows, int columns) {
		this(new Dimension(columns, rows), 0, 0);
	}
	 ...
	 * or horizontal padding.
	 */
	public KeyboardLayout(Dimension gridSize, int borderSZ) {
		this(gridSize, 0, 0);
	 ...
	 */
	public KeyboardLayout(Dimension gridSize, int hgap, int vgap) {
		if ((gridSize.width <= 0) || (gridSize.height <= 0)) { throw new IllegalArgumentException(
	...
	/**
	 * Returns the Constraints of a given Component. Returns null if the component
	 ...
	 *         container.
	 * @see java.awt.KeyboardLayout#getLargestCellSize
	 */
	protected Dimension getLayoutSize(Container parent, boolean isPreferred) {
		...
		largestSize.width = (largestSize.width * gridSize.width) + (hgap * (gridSize.width + 1)) + insets.left + insets.right;
		largestSize.height = (largestSize.height * gridSize.height) + (vgap * (gridSize.height + 1)) + insets.top + insets.bottom;
	...
	public void layoutContainer(Container parent) {
		synchronized (parent.getTreeLock()) {
			...
			if (ncomponents>0) {
				...
				int totalW = size.width - (insets.left + insets.right + borderSZ * 2);
				int totalH = size.height - (insets.top + insets.bottom);

				// Cell dimensions, without padding
				double cellW = (double)(totalW - ((gridSize.width - 1) * hgap)) / (double)gridSize.width;
				double cellH = (double)(totalH - ((gridSize.height - 1) * vgap)) / (double)gridSize.height;

				// Cell dimensions, including padding
				double totalCellW = cellW + hgap;
				double totalCellH = cellH + vgap;

				for (int i = 0; i < ncomponents; i++) {
					Component c = parent.getComponent(i);
					Rectangle rect = compTable.get(c);
					if (rect != null) {
						int x = insets.left + (int)(totalCellW * rect.x);
						int y = insets.top + (int)(totalCellH * rect.y);
						int w;
						int h;
						if (true) {
							w = insets.left + (int)(totalCellW * (rect.x+rect.width)) - x - hgap;
							h = insets.top + (int)(totalCellH * (rect.y+rect.height)) - y - vgap;
						} else {
							w = (int)(cellW * rect.width) + hgap * (rect.width - 1) + 1;
							h = (int)(cellH * rect.height) + vgap * (rect.height - 1) + 1;
						}
						c.setBounds(x, y, w, h);
					}
				}
	...
	public void addLayoutComponent(Component comp, Object constraints) {
		if (constraints instanceof Rectangle) {
			...
			if (rect.width <= 0 || rect.height <= 0) { throw new IllegalArgumentException(
					"cannot add to layout: rectangle must have positive width and height"); }
			if (rect.x < 0 || rect.y < 0) { throw new IllegalArgumentException(
					"cannot add to layout: rectangle x and y must be >= 0"); }
			setConstraints(comp, rect);
		} else if (constraints != null) { throw new IllegalArgumentException("cannot add to layout: constraint must be a Rectangle"); }
	}
