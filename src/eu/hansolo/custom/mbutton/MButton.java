 ...
 * @author grunwald
 */
public class MButton extends javax.swing.JButton implements
		java.awt.event.ComponentListener, java.awt.event.MouseListener,
		java.awt.event.MouseMotionListener {

	private static final long serialVersionUID = 119037003117821140L;
	...
	private int effectDuration = 250;
	...
	private static final org.pushingpixels.trident.ease.TimelineEase STANDARD_EASING = new org.pushingpixels.trident.ease.Spline(
			0.3f);
	private static final float[] EFFECT_FRACTIONS = { 0.0f, 0.15f, 1.0f };
//	private java.awt.Color effectColor = new java.awt.Color(255, 50, 0, 200);
	private java.awt.Color effectColor = new java.awt.Color(200, 200, 200, 200);
	private java.awt.Color[] effectColors = {
			effectColor,
			effectColor,
			new java.awt.Color(effectColor.getRed(), effectColor.getGreen(),
					effectColor.getBlue(), 0) };
//	private java.awt.Color effectColorPressed = new java.awt.Color(50, 161, 0, 200);
//	private java.awt.Color effectColorPressed = new java.awt.Color(220, 220, 220, 200);
	private java.awt.Color effectColorPressed = new java.awt.Color(176, 250, 250, 220);
	private java.awt.Color[] effectColorsPressed = {
			effectColorPressed,
			effectColorPressed,
			new java.awt.Color(effectColorPressed.getRed(),
					effectColorPressed.getGreen(),
					effectColorPressed.getBlue(), 0) };
...
//		setForeground(new java.awt.Color(0xCCCCCC));
	...
	public MButton(ImageIcon imgUp, ImageIcon imgDown) {
		this();
		super.setIcon(imgUp);
		super.setPressedIcon(imgDown);
	}
	// </editor-fold>
	...
	public void setText(String text) {
    	super.setText(text);
		setFont(hasUnicodeChar() ? new java.awt.Font("Courier New", 1, 24) : new java.awt.Font("Verdana", 0, 20));
    }

	private boolean hasUnicodeChar() {
	...
	public void setEffectColor(final java.awt.Color EFFECT_COLOR) {
		...
		this.effectColors = new java.awt.Color[] {
				effectColor,
				effectColor,
				new java.awt.Color(effectColor.getRed(),
						effectColor.getGreen(), effectColor.getBlue(), 0) };
		repaint();
	}
	...
	private java.awt.image.BufferedImage create_EFFECT_Image(final int WIDTH,
		...
		final java.awt.image.BufferedImage IMAGE = GFX_CONF
		...
		final java.awt.Rectangle BOUNDS = new java.awt.Rectangle(
				0,
				true||CLICKED ? -(int) (IMAGE_HEIGHT * 0.7) : (int) (IMAGE_HEIGHT * 0.6 / 2), 
				IMAGE_WIDTH,
				true||CLICKED ? (int) (IMAGE_HEIGHT * 1.4) : (int) (IMAGE_HEIGHT * 1.4)
		);
		final java.awt.RadialGradientPaint GRADIENT;

		if (CLICKED) {
			GRADIENT = new java.awt.RadialGradientPaint(BOUNDS,
					EFFECT_FRACTIONS, effectColorsPressed,
					java.awt.MultipleGradientPaint.CycleMethod.NO_CYCLE);
		} else {
			GRADIENT = new java.awt.RadialGradientPaint(BOUNDS,
					EFFECT_FRACTIONS, effectColors,
					java.awt.MultipleGradientPaint.CycleMethod.NO_CYCLE);
		}
	...
	public void mouseClicked(final java.awt.event.MouseEvent EVENT) {
		...
		if (isEnabled()) {
			...
			TIMELINE.setDuration(effectDuration);
	...
	public void mouseReleased(final java.awt.event.MouseEvent EVENT) {
		if (isEnabled()) {
			...
			TIMELINE.addPropertyToInterpolate("alpha", 0.0f, 1.0f);
			TIMELINE.setDuration(2*effectDuration);
			TIMELINE.playReverse();
		}
		repaint();
		mousePressed = false;
	...
	public void mouseEntered(final java.awt.event.MouseEvent EVENT) {
		...
		if (isEnabled() && alpha < 1.0f) {
			// Fade in the effect slowly (600 ms) if the mouse entered the
			// button
			if (TIMELINE.getState() != org.pushingpixels.trident.Timeline.TimelineState.IDLE) {
				TIMELINE.abort();
			}
	...
	public void mouseExited(final java.awt.event.MouseEvent EVENT) {
		...
		mouseOver = false;
		if (isEnabled()) {
			// Abort running timeline if it's not idle and button was not
			// clicked
			if (TIMELINE.getState() != org.pushingpixels.trident.Timeline.TimelineState.IDLE
					&& !mouseClicked) {
				TIMELINE.abort();
			 ...
			 * Because it will always fade from 1.0f - 0.0f it looks like the
			 * effect will be fade in completely before it fades out slowly. You
			 * could see it if you simply move the mouse over a button and
			 * directly out again. In this case it will set the alpha value of
			 * the effect image to 1.0f (opaque) and fades out slowly.
			 */
